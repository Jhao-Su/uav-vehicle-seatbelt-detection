...
与 v1 不同，v2 采用分段置信度策略：高置信度(>0.7)直接输出，
中置信度(0.4-0.7)经过车窗/安全带 IOU 二次验证后再输出。
...
def _load_detector():
    ...
    os.chdir(_BASE_DIR)
    try:
        _detector = importlib.import_module("seatbelt_detection_v2.seatbelt_detector")
    finally:
        os.chdir(_saved_cwd)
